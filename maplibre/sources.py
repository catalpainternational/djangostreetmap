...
class Source(BaseModel):
    ...
    tiles: list[AnyUrl] | None = Field(default_factory=list, description="An array of one or more tile source URLs, as in the TileJSON spec.")
...
class Vector(Source):
    ...
    promoteId: str | None = Field(
        None,
        description=(
            "A property to use as a feature id (for feature state). "
            "Either a property name, or an object of the form {<sourceLayer>: <propertyName>}. "
            "If specified as a string for a vector tile source, the same property is used across all its source layers."
        ),
    )
    url: AnyUrl | None = Field(None, description="A URL to a TileJSON resource.")
...
class GeoJson(Source):
    ...
    clusterMinPoints: int | None = Field(2, description="Minimum number of points necessary to form a cluster if clustering is enabled.")
    clusterProperties: dict[str, layer.Expression] | None = Field(
        None,
        description="""
    ...
    data: str | GeoJsonFeature | GeoJsonFeatureCollection = Field(description="A URL to a GeoJSON file, or inline GeoJSON.")
    filter: str | None = Field(None, description="An expression for filtering features prior to processing them for rendering.")
    ...
    promoteId: str | dict[str, str] | None = Field(
        None,
    ...
    tolerance: float | None = Field(None, description="Douglas-Peucker simplification tolerance (higher means simpler geometries and faster performance).")
...
AnySource = Annotated[Vector | Raster | RasterDem | GeoJson | Image | Video, Field(discriminator="type")]
